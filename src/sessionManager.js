...
class SessionManager {
  ...
  async getSession(token) {
    try {
      ...
      logger.debug(
        ...
        memorySession ? 'FOUND' : 'NOT FOUND',
      );
      if (memorySession) {
        // Check if memory session is expired
        if (
          memorySession.expiresAt &&
          new Date(memorySession.expiresAt).getTime() < Date.now()
        ) {
  ...
  async invalidateUserSessions(userId) {
    try {
      ...
      this.metrics.activeSessions = Math.max(
        0,
        this.metrics.activeSessions - invalidatedCount,
      );
      logger.system(
  ...
  async invalidateAllSessions() {
    try {
      ...
      logger.system(
        `Invalidated all sessions. Redis: ${redisCount}, Memory: ${memoryCount}`,
      );
  ...
  async cleanupExpired() {
    try {
      ...
      if (cleanedCount > 0) {
        ...
        this.metrics.activeSessions = Math.max(
          0,
          this.metrics.activeSessions - cleanedCount,
        );
      }
