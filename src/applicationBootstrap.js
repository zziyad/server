...
class ApplicationBootstrap {
  ...
  async loadCodeModules(application) {
    try {
      ...
      await Promise.all([lib.load(), domain.load(), config.load()]);
  ...
  async initializeServices() {
    try {
      ...
      if (metarhia.metawatch && metarhia.metawatch.DirectoryWatcher) {
        this.application.watcher = new metarhia.metawatch.DirectoryWatcher({
          timeout: 1000,
        });
  ...
  async executeStarts() {
    try {
      ...
      console.log(
        `Executing ${this.application.starts.length} startup functions...`,
      );
      ...
      const startPromises = this.application.starts.map((startFn, index) =>
        common
          ...
          .catch((error) => {
            console.error(
              `Startup function ${index + 1} failed:`,
              error.message,
            );
  ...
  async bootstrap() {
    try {
      ...
      const { config, lib, domain } = await this.loadCodeModules(
        this.application,
      );
  ...
  async shutdown() {
    try {
      ...
      if (
        ...
        typeof this.application.watcher.close === 'function'
      ) {
